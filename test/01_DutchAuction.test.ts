...
describe.only("01_DutchAuction", function () {
    ...
    it("success on checking owner", async () => {
        const owner = await nft.ownerOf(NFT_ID);
        expect(owner).to.be.eq(deployer.address);
    })

    it("fails on sending less amount", async function() {
        const price = await dutchAuction.getPrice();
        const tx = dutchAuction.connect(buyer).buy({value: price.toNumber() - 10});
        await expect(tx).to.be.revertedWith('ETH < price');
    })

    it("fails on buying after the auction ends", async function() {
        await  timeTraveller(7);
        const price = await dutchAuction.getPrice();
        const tx = dutchAuction.connect(buyer).buy({value: price});
        await expect(tx).to.be.revertedWith('auction over');
    })
